...
export class ConsultarAnticiposComponent implements OnInit {
  columnsDetalle: string[] = ['Titulo', 'Solicitante', 'FechaSolicitud', 'FechaFinalizacion', 'FechaLegalizacion', 'Anticipo', 'Legalizacion', 'Reportes', 'Facturas'];
  anticipos = [];
  modalRef: BsModalRef;
  data = [];
  resumen = [];
  ...
  constructor(public Servicios: ServiciosService, private modalService: BsModalService, public router: Router, public toastr: ToastrService) { }
  ...
  ConstruirArrayDatos(id: number) {
    ...
    //totales gastos
    this.totalGastosPesos = this.SumarTotales(arrGastos, 'Peso');
    this.totalGastosDolares = this.SumarTotales(arrGastos, 'Dolar');
    this.totalGastosEuros = this.SumarTotales(arrGastos, 'Euro');
    //totales saldos
    this.totalSaldoPesos = this.SumarTotales(arrSaldos, 'Peso');
    this.totalSaldoDolares = this.SumarTotales(arrSaldos, 'Dolar');
    this.totalSaldoEuros = this.SumarTotales(arrSaldos, 'Euro');
    //saldo a favor
  ...
  async GenerafPdfLegalizacion() {
    ...
    pdf.add(
      new Txt(`Anticipo o Reintegro: Consecutivo ${this.dataInicial[0].Consecutivo}`).fontSize(18).bold().alignment('center').margin([10, 5]).end
    )
    pdf.add(
    ...
    pdf.pageSize("A4");
    pdf.pageMargins([40, 100, 40, 5]);
    pdf.footer('Esto es un footer')
